...
public class SonyServlet extends HttpServlet {
    ...
    throws ServletException, java.io.IOException {
        ...
        out.println(docType +
                ...
                "<li class='start selected'><a href='#'>Sony</a></li>"+
                "</ul>"+
                "</nav>"+
                "<img class='header-image' src='images/img_PlayStation.jpg' width = '100%' height = '100%' alt='X Box' />"+
                "<div id='body'>"+
                "<section id='content'>"+
                "<article>"+
                "<h2>X Box</h2>"+
                "<p>Following are the different models of PlayStation for sale.</p>"+
                "<p>Click on a model to purchase or check the reviews.</p>"+
                "</article>"+
                "<article class='expanded'>"+
                "<h2>Models in PlayStation</h2>"+
                "<table>");
        for(ProdDS element : listx)
        {
            if(element.getProdManfName().equals("Sony"))
            {
               ...
               // double origPrice = Double.valueOf(originalPrice);
                ...
                out.println("<tr>");
                out.println("<td>");
                out.println("<img src = 'images/img_PlayStation4.jpg' width = '200' height = '200' alt = 'PlayStation'>");
                out.println("</td>");
                out.println("<td>");
                ...
                out.println("<input class = 'submit-button' type = 'submit' name = 'SonyPS' value = 'Add to Cart'>");
                out.println("</form>");
                ...
                out.println("<input class = 'submit-button' type = 'submit' name = 'SonyPS' value = 'Write Review'>");
                out.println("</form>");
                out.println("<form class = 'submit-button' method = 'get' action = '/csj/ViewReviews'>");
                out.println("<input type = 'hidden' name = 'hiddenProdID' value = '"+element.getID()+"'>");
                out.println("<input class = 'submit-button' type = 'submit' name = 'SonyPS' value = 'View Reviews'>");
                out.println("</form>");
                out.println("</td>");
                out.println("</tr>");
            }
